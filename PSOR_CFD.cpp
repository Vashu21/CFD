...
double omega, alpha, q_1, q_2, bet, x[Nx], y[Ny], es = 0, err = 1, dx = 0.05, dy = 0.05, u[Nx][Ny], ukpl[Nx][Ny];
/* bet is Beta parameter ; es dummy variable for error (err) ; errmx Maximum error value ; dx,dy Grid spacing ; u Temperature array */
bet = dx / dy;
q_1 = (M_PI)/(Nx - 1);
q_2 = (M_PI)/(Ny - 1);
alpha = ((cos(q_1))+(bet*bet*cos(q_2)))/(1+(bet*bet)); alpha = pow(alpha,2);
...
for (j = 1; j < (Ny - 1); j++)
{
ukpl[i][j] = ((1 - omega)*ukpl[i][j]) + omega*((ukpl[i + 1][j] + ukpl[i - 1][j] + (bet * bet * (ukpl[i][j + 1] + ukpl[i][j - 1]))) / (2 * (1 + (bet * bet))));
/* PSOR*/
}
}
for (i = 0; i < Nx; i++)
...
FILE *g,*p;
g=fopen("PSOR.dat","w"); fprintf(g,"Variables=\"X\",\"Y\",\"Temprature\"\n",Ny,Nx); fprintf(g,"\tI=%d\tJ=%d\n",Nx,Ny);
for(i = 0; i < Nx; i++)
{
for(j = 0; j < Ny; j++)
{
...
fclose(g); p=fopen("PSOR_nodes.dat","w"); for(j = 0; j < Ny; j++)
{
fprintf(p,"%d\t%d\t%lf\n",11,j+1,u[10][j]);
